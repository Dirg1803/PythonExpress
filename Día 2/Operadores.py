x=6
y=2
z=7

print(f"{x} mas {y} es igual a {x+y}")
print(f"{x} menos {y} es igual a {x-y}")
print(f"{x} por {y} es igual a {x*y}")
print(f"{x} entre {y} es igual a {x/y}")
print(f"{z} entre {y} es igual a {z/y}")


print(f"{z} dividiendo al piso {y} es igual {z//y}")
print(f"{z} modulo de {y} es igual {z%y}")
print(f"{x} elevado a la {y} es igual a {x**y}")
print(f"{x} elevado a la {3} es igual a {x**y}")
print(f"la raiz cuadrada de {64} es {64**0.5}")